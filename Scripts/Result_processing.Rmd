...
aggr.names.housekeeping <- c(aggr.names, rep("DNA_gyrase_A", 6), rep("DNA_gyrase_B", 6), rep("Recombinase_A", 6), rep("RNA_polymerase_A",6), rep("RNA_polymerase_B",6))
```


```{r}
## aggregating the pathways:

# vsensitive
rownames(mat.vsensitive) <- aggr.names.housekeeping
short.mat.vsensitive <- aggregate(mat.vsensitive, list(aggr.names.housekeeping), "sum")

# sensitive
rownames(mat.sensitive) <- aggr.names.housekeeping
short.mat.sensitive <- aggregate(mat.sensitive, list(aggr.names.housekeeping), "sum")

# fast
rownames(mat.fast) <- aggr.names.housekeeping
short.mat.fast <- aggregate(mat.fast, list(aggr.names.housekeeping), "sum")

# vfast
rownames(mat.vfast) <- aggr.names.housekeeping
short.mat.vfast <- aggregate(mat.vfast, list(aggr.names.housekeeping), "sum")

# oral
rownames(mat.oral) <- aggr.names.housekeeping
short.mat.oral <- aggregate(mat.oral, list(aggr.names.housekeeping), "sum")
```
